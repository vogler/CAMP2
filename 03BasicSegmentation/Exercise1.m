...
Image_nf = convn(Image_n, fspecial('gaussian', 11, 1.5)); % Filtered version of "Image_n"
...
%Image_PostProcess = Image_nf.*(Image_nf_km==2);
Image_PostProcess = zeros(size(Image_nf));
Image_PostProcess(Image_nf_km==2) = 1;
%morph = strel([1 1 1]);
%morph = strel('square', 3);
morph = [0 1 0; 1 1 0; 0 0 0];
Image_PostProcess = imclose(imopen(Image_PostProcess, morph), morph);
%Image_PostProcess = imdilate(Image_PostProcess, strel([1 1 1]));
%Image_PostProcess = imdilate(Image_PostProcess, strel([1;1;1]));
%Image_PostProcess = imerode(Image_PostProcess, strel([1 1 1]));
%Image_PostProcess = imerode(Image_PostProcess, strel([1;1;1]));
