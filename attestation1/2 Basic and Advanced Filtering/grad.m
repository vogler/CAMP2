...
% the last coloumn has zeros
% subtract the next column value (j+1) from the current (j)

%approximate the derivative of u in y-direction with forward differences
dyu(1:(m-1),:) =  u(2:m, :) - u(1:(m-1), :);
% the last row has zeros
% subtract the next row value (i+1) from the current (i)

% Neumann condition: outer border = 0
dxu(:, n) = u(:, n);
dyu(m, :) = u(m, :);