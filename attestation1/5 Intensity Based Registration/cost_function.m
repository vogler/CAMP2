...
switch upper(similarity_measure)
    ...
    case 'NCC'
        ...
        similarity_value = -abs(mean2((double((img_fixed-m1).*(img_registertest-m2)))./(s1*s2)));
    ...
    case 'MI'
        ...
        p_x = hist(double(img_fixed(:)), bins); % histogram for img_fixed
        p_y = hist(double(img_registertest(:)), bins); % histogram for img_registertest
        tmp = zeros(bins);
        for i=1:bins
            for j=1:bins
                ...
                if(((p_x(i) * p_y(j)) ~= 0) && (p_xy(i,j)/(p_x(i)* p_y(j)) ~= 0))
                    tmp(i,j) = p_xy(i,j) * log(p_xy(i,j)/(p_x(i)* p_y(j))); %formula
                end
            end
        end
