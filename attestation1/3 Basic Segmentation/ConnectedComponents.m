...
for i=2:size(Image,1)
    for j=2:size(Image,2)
        ...
        if (Image(i,j)==1) % image is binary
            %% Check the neighbors at the top and at the left
            %% and assign their label to n1 and n2
            n1 = Cluster(i,j-1); %left
            n2 = Cluster(i-1,j); %top
...
p=1;
for k=1:NewLabel
    if sum(sum(Cluster==k))>0 %%check if there are empty Clusters
        Cluster(Cluster==k)=p;%%set valid cluster to p
        p=p+1;
    end
end
